...
public interface StockListItemsRepo extends CrudRepository<StockListItems, Integer> {
    ...
    @Query(value = "SELECT * FROM stock_list_items WHERE stock_list_id = ?1", nativeQuery = true)
    List<StockListItems> findByStockListId(Integer stockListId);
    
    @Query(value = "SELECT * FROM stock_list_items WHERE stock_list_id = ?1 AND symbol = ?2", nativeQuery = true)
    List<StockListItems> findByStockListIdAndSymbol(Integer stockListId, String symbol);
    
    @Modifying
    @Transactional
    @Query(value = "DELETE FROM stock_list_items WHERE stock_list_id = ?1 AND symbol = ?2", nativeQuery = true)
